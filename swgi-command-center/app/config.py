...
class Settings(BaseModel):
    ...
    auth_cookie_name: str = Field(default_factory=lambda: os.getenv("AUTH_COOKIE_NAME", "swgi_cc_session"))
    auth_cookie_secure: bool = Field(default_factory=lambda: os.getenv("AUTH_COOKIE_SECURE", "false").lower() == "true")
    auth_cookie_samesite: str = Field(default_factory=lambda: os.getenv("AUTH_COOKIE_SAMESITE", "lax"))
    login_failure_limit: int = Field(default_factory=lambda: int(os.getenv("LOGIN_FAILURE_LIMIT", "5")))
    login_lockout_minutes: int = Field(default_factory=lambda: int(os.getenv("LOGIN_LOCKOUT_MINUTES", "15")))
